...
select sum(department) as total from employees;
select department, count(*) as numofemployers from employees group by department;
select department,count(*) from employees as numofemployers group by department having count(*)>2;
select name from departments order by budget desc limit 1 offset 1;
select e.name,e.lastname from employees e join departments d on d.code = e.department where d.budget=(
